...
console.log(mongoose.connect(process.env.MONGO_URI, { useNewUrlParser: true, useUnifiedTopology: true })); 
...
var createAndSavePerson = function(done) {
  var grimesWong = new Person({name: "Grimes Wong", age: 40, favoriteFoods: ["chicken wings", "pork chops", "French fries"]});
  
  grimesWong.save(function (err, data) {
...
var arrayOfPeople = [{name: "Arthur", age: 39, favoriteFoods: ["Curry chicken", "burger"]}, {name: "Karen", age: 33, favoriteFoods: ["Veggies", "fruits"]}, {name: "Rex", age: 7, favoriteFoods: ["Ice scream", "pizza"]}]
...
var findEditThenSave = function(personId, done) {
  ...
  Person.findById({_id: personId}, function(err, data) {
    ...
    data.markModified('favoriteFoods');  // tell the monogoose the mixed type has changed
...
var queryChain = function(done) {
  ...
  Person.find({favoriteFoods: foodToSearch}).sort({ name: 'asc'}).limit(2).select('-age').exec(done)
